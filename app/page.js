...
export default function HomePage() {
  ...
  function printHoaReport(hoa) {
    ...
    win.document.write(`
      ...
      <table>
        <tr><th>Job #</th><th>Job Name</th><th>Address</th><th>Status</th><th>Submitted</th><th>Approved</th><th>Notes</th></tr>
        ${hoaJobs.map(j => `<tr><td>${esc(j.job_number)}</td><td>${esc(j.job_name)}</td><td>${esc(j.address)}</td><td>${esc(j.status)}</td><td>${esc(j.date_submitted)||''}</td><td>${esc(j.date_approved)||''}</td><td>${esc(j.notes)}</td></tr>`).join('')}
      </table>
      </body></html>
    `);
    win.document.close();
  ...
  const filteredHoas = hoas.filter(h => {
    if (!q) return true;
    const jobMatch = jobs.some(j => j.hoa_id === h.id && (
      j.address?.toLowerCase().includes(q) || j.job_number?.toLowerCase().includes(q) || j.job_name?.toLowerCase().includes(q)
    ));
    return h.name.toLowerCase().includes(q) || (h.mgmt_co || '').toLowerCase().includes(q) ||
      (h.contact_name || '').toLowerCase().includes(q) || (h.address || '').toLowerCase().includes(q) || jobMatch;
  });
  ...
  return (
    <>
      ...
      <div className="stats-bar">
        {[['Total HOAs', statCounts.total], ['Total Jobs', statCounts.totalJobs],
          ...JOB_STATUSES.map(s => [s, statCounts[s]])].map(([label, num]) => (
      ...
      {view === 'hoas' ? (
        <div className="layout">
          <div className="panel-list">
            <div className="search-box">
              <input placeholder="Search HOA, contact, job address..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
        ...
        <div className="layout">
          <div className="panel-detail" style={{ flex: 1 }}>
            ...
            <table>
              <thead>
                <tr><th>Job #</th><th>Job Name</th><th>HOA</th><th>Address</th><th>Status</th><th>Submitted</th><th>Approved</th><th></th></tr>
              </thead>
              <tbody>
                {jobRows.length === 0 && <tr><td colSpan={8} style={{ color: '#999' }}>No jobs match this filter.</td></tr>}
                {jobRows.map(j => {
  ...
  return (
    <>
      ...
      <section className="block">
        ...
        <table>
          <thead><tr><th>Job #</th><th>Job Name</th><th>Address</th><th>Status</th><th>Submitted</th><th>Approved</th><th></th></tr></thead>
          <tbody>
            {jobs.length === 0 && <tr><td colSpan={7} style={{ color: '#999' }}>No jobs linked yet.</td></tr>}
            {jobs.map(j => (
              <tr key={j.id}>
                <td>{j.job_number || '—'}</td>
                <td>{j.job_name || '—'}</td>
...
function JobModal({ job, hoas, onSave, onClose }) {
  const [form, setForm] = useState({
    ...
    status: job.status || 'Need to Submit', date_submitted: job.date_submitted || '', date_approved: job.date_approved || '',
    notes: job.notes || ''
  });
  const set = (k) => (e) => setForm({ ...form, [k]: e.target.value });
  return (
      ...
      <div className="modal">
        ...
        <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
          {mode === 'hoa'
            ...
            : 'Columns: hoaName, jobNumber, jobName, address, status, dateSubmitted, dateApproved, notes. hoaName must match an existing HOA.'}
        </p>
