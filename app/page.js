...
export default function HomePage() {
  ...
  function printHoaReport(hoa) {
    ...
    win.document.write(`
      ...
      <table>
        <tr><th>Job #</th><th>Job Name</th><th>Address</th><th>Status</th><th>Assigned</th><th>Submitted</th><th>Approved</th><th>Notes</th></tr>
        ${hoaJobs.map(j => `<tr><td>${esc(j.job_number)}</td><td>${esc(j.job_name)}</td><td>${esc(j.address)}</td><td>${esc(j.status)}</td><td>${esc(j.assigned_date)||''}</td><td>${esc(j.date_submitted)||''}</td><td>${esc(j.date_approved)||''}</td><td>${esc(j.notes)}</td></tr>`).join('')}
      </table>
      </body></html>
    `);
    win.document.close();
  ...
  function printJobsReport(rows, filterLabel) {
    ...
    win.document.write(`
      ...
      <table>
        <tr><th>Job #</th><th>Job Name</th><th>HOA</th><th>Address</th><th>Status</th><th>Assigned</th><th>Submitted</th><th>Approved</th><th>Notes</th></tr>
        ${rows.map(j => {
          const hoa = hoas.find(h => h.id === j.hoa_id);
          return `<tr><td>${esc(j.job_number)}</td><td>${esc(j.job_name)}</td><td>${esc(hoa ? hoa.name : 'Unlinked')}</td><td>${esc(j.address)}</td><td>${esc(j.status)}</td><td>${esc(j.assigned_date)||''}</td><td>${esc(j.date_submitted)||''}</td><td>${esc(j.date_approved)||''}</td><td>${esc(j.notes)}</td></tr>`;
        }).join('')}
      </table>
      </body></html>
    `);
          ...
          <div className="panel-detail" style={{ flex: 1 }}>
            <div className="detail-header">
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <button className="btn-outline btn-sm" onClick={() => printJobsReport(jobRows, jobsFilter === 'All' ? 'All Jobs' : jobsFilter)}>Print Report</button>
            ...
            <table>
              <thead>
                <tr><th>Job #</th><th>Job Name</th><th>HOA</th><th>Address</th><th>Status</th><th>Assigned</th><th>Submitted</th><th>Approved</th><th>Notes</th><th></th></tr>
              </thead>
              <tbody>
                {jobRows.length === 0 && <tr><td colSpan={10} style={{ color: '#999' }}>No jobs match this filter.</td></tr>}
                {jobRows.map(j => {
                  const hoa = hoas.find(h => h.id === j.hoa_id);
                  return (
                    <tr key={j.id}>
  ...
  return (
    <>
      ...
      <section className="block">
        ...
        <table>
          <thead><tr><th>Job #</th><th>Job Name</th><th>Address</th><th>Status</th><th>Assigned</th><th>Submitted</th><th>Approved</th><th>Notes</th><th></th></tr></thead>
          <tbody>
            {jobs.length === 0 && <tr><td colSpan={9} style={{ color: '#999' }}>No jobs linked yet.</td></tr>}
            {jobs.map(j => (
              <tr key={j.id}>
                <td>{j.job_number || '—'}</td>
                <td>{j.job_name || '—'}</td>
...
function JobModal({ job, hoas, onSave, onClose }) {
  const [form, setForm] = useState({
    ...
    status: job.status || 'Need to Submit', assigned_date: job.assigned_date || '',
      ...
      <div className="modal">
        ...
        <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
          {mode === 'hoa'
            ...
            : 'Columns: hoaName, jobNumber, jobName, address, status, assignedDate, dateSubmitted, dateApproved, notes. hoaName must match an existing HOA.'}
        </p>
