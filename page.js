      ...
      {view === 'hoas' ? (
        ...
        <div className="layout">
          <div className="panel-detail" style={{ flex: 1 }}>
            ...
            <table>
              <thead>
                <tr><th>Job #</th><th>Job Name</th><th>HOA</th><th>Address</th><th>Status</th><th>Assigned</th><th>Submitted</th><th>Approved</th><th></th></tr>
              </thead>
              <tbody>
                {jobRows.length === 0 && <tr><td colSpan={9} style={{ color: '#999' }}>No jobs match this filter.</td></tr>}
                {jobRows.map(j => {
                  const hoa = hoas.find(h => h.id === j.hoa_id);
                  return (
                    <tr key={j.id}>
  ...
  return (
    <>
      ...
      <section className="block">
        ...
        <table>
          <thead><tr><th>Job #</th><th>Job Name</th><th>Address</th><th>Status</th><th>Assigned</th><th>Submitted</th><th>Approved</th><th></th></tr></thead>
          <tbody>
            {jobs.length === 0 && <tr><td colSpan={8} style={{ color: '#999' }}>No jobs linked yet.</td></tr>}
            {jobs.map(j => (
              <tr key={j.id}>
                <td>{j.job_number || '—'}</td>
                <td>{j.job_name || '—'}</td>
